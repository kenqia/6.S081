...
e1000_transmit(struct mbuf *m)
{
  ...
  //
  acquire(&e1000_lock);
  int tx_head = regs[E1000_TDH];
  int tx_tail = regs[E1000_TDT];
  if(tx_head == (tx_tail + 1) % TX_RING_SIZE || (tx_ring[tx_tail].status & E1000_TXD_STAT_DD) == 0){
    release(&e1000_lock);
    return -1;
  }
  ...
  tx_ring[tx_tail].length = (uint16)m->len;
...
e1000_recv(void)
{
  ...
  while(rx_ring[next].status & E1000_RXD_STAT_DD){
    ...
    if((rx_mbufs[next] = mbufalloc(0)) == 0){
      panic("mbfalling");
    }
  ...
  regs[E1000_RDT] = ((next - 1 + RX_RING_SIZE) % RX_RING_SIZE);
}
